...
**모드가 남기는 파일은 셋이 아니라 하나입니다.** 설정과 실시간 모터 세기가 `reframework/data/WildsDualSenseBTRumble.txt` 한 파일을 같이 쓰고, 값마다 고유한 이름이 붙어 있어서 둘을 서로 오인할 수 없습니다. 설정이 앞에, state 줄이 맨 뒤에 옵니다. lua는 파일을 잘라내고 다시 쓰기 때문에 읽는 쪽이 중간에 걸리면 앞부분만 보게 되는데, 모든 값에 이름이 붙어 있으면 잘린 state 줄은 반쯤 읽히는 대신 그냥 버려집니다. 플러그인은 자체 로그를 남기지 않고 REFramework 로그에, 그것도 상태가 바뀔 때만 기록합니다. v1.0에서 올라오면 예전에 남기던 파일 둘 다 정리됩니다. lua가 기존 `.json`을 읽어들인 뒤 지우고, 플러그인은 죽은 `.log`를 지웁니다. lua 쪽 삭제가 실패했을 경우를 대비해, 플러그인은 lua가 새 형식으로 쓰는 걸 확인한 다음 `.json`도 한 번 더 지웁니다.
