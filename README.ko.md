...
**모드가 남기는 파일은 셋이 아니라 하나입니다.** `reframework/data/WildsDualSenseBTRumble.json` 안에 저장된 설정이 `settings`로, 실시간 모터 세기가 `state`로 들어갑니다. v1.0이 설정을 넣어두던 바로 그 파일이라, 업데이트해도 옮길 게 없고 설정은 그대로 따라옵니다. 설정이 앞에 오고 `state`는 한 줄을 통째로 씁니다. lua는 파일을 잘라내고 다시 쓰기 때문에 읽는 쪽이 중간에 걸리면 앞부분만 보게 되는데, `state`가 한 줄이면 통째로 읽히거나 아예 버려지거나 둘 중 하나가 됩니다. 플러그인은 자체 로그를 남기지 않고 REFramework 로그에, 그것도 상태가 바뀔 때만 기록합니다. v1.0이 그 폴더에 남기던 `.txt`와 `.log`는 업데이트할 때 둘 다 삭제됩니다.
